...
exports.login = catchAsync(async (req, res, next) => {
    ...
    if (!user || !await user.correctPassword(password, user.password)) {
        return next(new AppError("Please enter correct email & password", 401, res));
    }
...
exports.protect = catchAsync(async (req, res, next) => {
    ...
    if (!currentUser) {
        next(new AppError('the User beloging to this token No Longer Exist', 401, res))
    }
...
exports.restrict = (...roles) => {
    return (req, res, next) => {
        if (!roles.includes(req.user.roles)) {
            return next(new AppError('You are not Authorized to perform this action', 403, res))
        }
        next()
    }
};
...
exports.resetPassword = catchAsync(async (req, res, next) => {
    ...
    const user = await User.findOne({
        passwordResetToken: encryptPasswordResetToken,
        passwordResetTokenExpire: { $gt: Date.now() }
    })
    if (!user) {
        return next(new AppError('Forget Password link has Expired', 400, res))
    }
...
exports.updatePassword = catchAsync(async (req, res, next) => {
    ...
    if (!user || !await user.correctPassword(req.body.passwordCurrent, user.password)) {
        return next(new AppError("Please entered correct password", 401, res));
    }
    user.password = req.body.password;
    user.passwordConfirm = req.body.passwordConfirm;
    await user.save()
